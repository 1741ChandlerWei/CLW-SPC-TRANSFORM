...
export async function buildOutputExcel(spec: ParsedSpec): Promise<Buffer> {
  ...
  ws.getColumn(1).width = 12  // 類別
  ws.getColumn(2).width = 26  // 項目
  ...
  // Sub: blank + 項目 | model names
  // Actually per example: col1=blank merged, col2="項目" is not shown in row2
  // Row2 just has 型號 in col1-2 merged, then model names
  models.forEach((m, i) => {
  ...
  applyHeader(ws.getCell(r, 2), false)
  ...
  allParts.forEach((part, pi) => {
    ...
    models.forEach((m, i) => {
      applyValue(ws.getCell(r, 3 + i), pi % 2 === 1)
      ws.getCell(r, 3 + i).value = m.parts[part] ?? ''
    })
    r++
  })
  ...
  allParts.forEach((part, pi) => {
    ...
    models.forEach((m, i) => {
      applyValue(ws.getCell(r, 3 + i), pi % 2 === 1)
      ws.getCell(r, 3 + i).value = m.density_imperial[part] ?? ''
    })
    r++
  })
  ...
  allParts.forEach((part, pi) => {
    ...
    models.forEach((m, i) => {
      applyValue(ws.getCell(r, 3 + i), pi % 2 === 1)
      ws.getCell(r, 3 + i).value = m.density_metric[part] ?? ''
    })
    r++
  })
...
// ─────────────────────────────────────────────
// Compare output: same layout, A版/B版 並排
// ─────────────────────────────────────────────
export async function buildCompareExcel(
  ...
  allModelNames.forEach((name, mi) => {
    ...
    applyHeader(ws.getCell(r, 3 + mi * 2), false)
  ...
  allModelNames.forEach((_, mi) => {
    ...
    applyHeader(ws.getCell(r, 4 + mi * 2), false)
  ...
  function writeCompRow(
    ...
    allModelNames.forEach((name, mi) => {
      ...
      const cA = ws.getCell(r, 3 + mi * 2)
      const cB = ws.getCell(r, 4 + mi * 2)
  ...
  allParts.forEach((part, pi) => {
    writeCompRow(part, '重量', false,
      m => m?.parts[part] ?? null, m => m?.parts[part] ?? null, pi % 2 === 1)
  })
  ...
  allParts.forEach((part, pi) => {
    writeCompRow(part, '密度  (英制)', false,
      m => m?.density_imperial[part] ?? null, m => m?.density_imperial[part] ?? null, pi % 2 === 1)
  })
  ...
  allParts.forEach((part, pi) => {
    writeCompRow(part, '密度  (公制)', false,
      m => m?.density_metric[part] ?? null, m => m?.density_metric[part] ?? null, pi % 2 === 1)
  })
